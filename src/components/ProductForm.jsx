...
export default function ProductForm({ product, onSaved }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    const payload = { title, slug, description, price: Math.round(Number(price)*100), images }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex gap-2">{images.map((u,i)=> <img key={i} src={u} className="w-20 h-20 object-cover rounded" />)}</div>
